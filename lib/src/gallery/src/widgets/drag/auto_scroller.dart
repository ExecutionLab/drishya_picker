...
import 'package:drishya_picker/src/gallery/src/widgets/drag/auto_scroll.dart';
...
class AutoScroller {
  ...
  bool get mustScroll => autoScroll.stopEvent.isNotConsumed || autoScroll.isScrolling;

  /// Returns the position in which the [controller] would be after performing
  /// an overscroll.
  @visibleForTesting
  double get positionAfterOverscroll => autoScroll.direction == AutoScrollDirection.forward
  ...
  double get targetPositionOfTheAutoScroll =>
      autoScroll.direction == AutoScrollDirection.forward ? controller.position.maxScrollExtent : 0;
  ...
  @visibleForTesting
  bool get isAbleToScroll => currentPosition != null && autoScroll.direction != null;
  ...
  bool get hasAnythingLeftToScroll =>
      ...
      ((autoScroll.direction == AutoScrollDirection.backward) && (currentPosition! > 0));
  ...
  Duration calculateScrollDurationWithUniformScrollSpeed(
    ...
    final scrollDurationInMs = (amountToBeScrolled * scrollDurationPerPixelInMs).toInt();
